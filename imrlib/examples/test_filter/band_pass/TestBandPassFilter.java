...
import imr.sound.audio.filter.BandPassFilter;
...
// Filter first frame
Filter filter = new BandPassFilter();
filter.setSize(FILTER_SIZE);
filter.setSampleRate(sample_rate);
filter.setBand(new Range(400.0f, 700.0f));
...
System.out.println("Saving audio file ... -> ../../sound/band_pass.wav");
...
AudioInputStream output = new AudioInputStream(new ByteArrayInputStream(buffer.toByteArray()), input.getFormat(),
	...
	try
	{
	AudioFileIO.store(output, "../../sound/band_pass.wav");
}
catch(UnsupportedAudioFileException e)
{
	e.printStackTrace();
