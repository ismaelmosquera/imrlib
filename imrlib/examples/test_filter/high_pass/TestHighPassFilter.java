...
import imr.sound.audio.filter.HighPassFilter;
...
filter.setCutOffFrequency(CUT_OFF_FREQUENCY);
...
System.out.println("Saving audio file ... -> ../../sound/high_pass.wav");
...
AudioInputStream output = new AudioInputStream(new ByteArrayInputStream(buffer.toByteArray()), input.getFormat(),
	...
	try
	{
	AudioFileIO.store(output, "../../sound/high_pass.wav");
}
catch(UnsupportedAudioFileException e)
{
	e.printStackTrace();
...
private static final float CUT_OFF_FREQUENCY = 600.0f;
