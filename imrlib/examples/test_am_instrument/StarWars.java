...
import imr.media.audio.AMInstrument;
...
* To illusttrate that, we composed a fragment of 'Stars Wars' and synthesized it using AM modulation.
...
public StarWars()
{
_am = new AMInstrument(1.0f, 20.0f, 44100.0f);
_player = new RawDataPlayer(_am.getCarrier().getSampleRate());
_saver = new RawDataStorage(_am.getCarrier().getSampleRate());
}

public void play()
{
	_am.getCarrier().setEnvADSR(10, 20, 60, 10);
	_am.getModulator().setAmplitude(1.0f);
	_am.getModulator().setEnvADSR(10, 25, 60, 5);
	_am.getModulator().setEnvSustainLevel(0.7f);
	_am.getModulator().setWaveType(WaveType.wTriangular);
// just to illustrate this example
_am.getModulator().setModulationIndex(5.0f);

_player.start();

int srate = (int)_am.getCarrier().getSampleRate();
byte[] frame;
...
if(a[i] != 0.0f)
{
_am.getCarrier().setAmplitude(a[i]);
_am.getCarrier().setFrequency(f[i]);
_am.getModulator().setFrequency(2.0f);
_am.synth(frame);
}
_player.play(frame);
_saver.add(frame);
