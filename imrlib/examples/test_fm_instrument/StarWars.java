...
_player = new RawDataPlayer(_fm.getSampleRate());
_saver = new RawDataStorage(_fm.getSampleRate());
}

public void play()
{
	_fm.setCarrierEnvADSR(10, 20, 60, 10);
	_fm.setModulatorAmplitude(1.0f);
	_fm.setModulatorEnvADSR(5, 25, 65, 5);
	_fm.setModulatorEnvSustainLevel(0.5f);
	_fm.setModulatorWaveType(WaveType.wSine);
// just to illustrate this example
_fm.setModulationIndex(4.0f);

_player.start();

int srate = (int)_fm.getSampleRate();
byte[] frame;
...
if(a[i] != 0.0f)
{
_fm.setCarrierAmplitude(a[i]);
_fm.setCarrierFrequency(f[i]);
_fm.setModulatorFrequency(2.0f);
_fm.synth(frame);
}
_player.play(frame);
_saver.add(frame);
