...
int n;
double sign = (get_sign(m) == NEGATIVE) ? -1.0 : 1.0;
...
if(min == LEFT_SIDE)
{
	...
	for(int i = 0; i < n; i++)
	{
_sigma.set(i, i, sign*Math.sqrt(left[i].value()));
}
}
else
{
	n = right.length;
		for(int i = 0; i < n; i++)
		{
	_sigma.set(i, i, sign*Math.sqrt(right[i].value()));
	}
}
return true;
}
...
private int x_sign(double x)
{
return (x < 0.0) ? NEGATIVE : POSITIVE;
}

private int get_sign(Matrix m)
{
int i, j, ret, s;
if(m.rows() != m.columns())
{
	ret = x_sign(m.get(0, 0));
	s = ret;
	for(i = 0; i < m.rows(); i++)
	{
	for(j = 0; j < m.columns(); j++)
	{
		if(x_sign(m.get(i, j)) !=ret)
		{
			s = x_sign(m.get(i, j));
			break;
		}
