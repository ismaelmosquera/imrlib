...
*
* @throws javax.sound.sampled.UnsupportedAudioFileException Unsupported audio file format.
*/
...
/**
* Sets a <code>Slot1</code> object to wrap to an encapsulated <code>Signal1</code> object.
* <p>
* The data emitted and received is just an array of float ( float[] ) while recording or playing.
* <p>
* This can be useful, for example, to plot the produced data while playing or recording.
* <p>
* Anyway, if you do not set any slot it does not matter, since it is not necessary for just recording or playing the produced data.
* <p>
* @param slot a <code>Slot1</code> with a float[] as generic parameter.
*
*/
public void setSlot(Slot1<float[]> slot)
{
	_signal.wrap(slot);
}
...
if(bytesRead > 0)
{
	_signal.emit(FrameFactory.getFrame(data));
_recBuffer.write(data, 0, bytesRead);
}
...
_signal.emit(FrameFactory.getFrame(_recBuffer.toByteArray()));
}
}
...
while(totalBytesRead < _totalBytesRec && !_stopRequested)
{
	bytesRead = input.read(data,0,_bufferSize);
		...
		{
			_signal.emit(FrameFactory.getFrame(data));
		_player.write(data,0,bytesRead);
	}
