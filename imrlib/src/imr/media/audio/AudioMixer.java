...
*
* @see imr.media.audio.FrameFactory
*
* @author Ismael Mosquera Rivera
*
*/
...
float max = getMax(mixed_track);
for(int i = 0; i < _track_size; i++) mixed_track[i] /= max;
// build the mixed track
byte[] out_track = new byte[_track_size];
// scale it to be in the range [-127 .. 127]
for(int i = 0; i < _track_size; i++) out_track[i] = (byte)(mixed_track[i] * 127.0f);
